...
const sectionInfoObject = {
  SRL: {
    name: 'Suggested Reviewed List',
    code: '19'
  }
};


moduleFor('controller:proposal/suggested-reviewers', 'Unit | Controller | proposal/suggested-reviewers', {
...
test('breadcrumb set', function(assert) {
  const controller = this.subject();
  assert.equal(controller.breadCrumb, 'List of Suggested Reviewers');
});
...
test('section lookup', function(assert) {
  ...
  assert.equal(controller.get('sectionInfo').name, 'Suggested Reviewed List');
  assert.equal(controller.get('sectionInfo').code, '19');
});
